...
error text_editor__handle_mouse(text_editor_t *self,
    ...
    text_editor_position_t pos;
    pos.line_index = mouse_event->y + self->first_visible_line;
    pos.column_index = mouse_event->x + self->first_visible_column;
    text_editor__clamp_position(self, &pos);
...
error text_editor__scroll(text_editor_t *self,
    ...
    if (command->mode == TEXT_EDITOR_SCROLL_MODE_ABSOLUTE) {
        self->first_visible_line = command->absolute_first_line;
        self->first_visible_column = command->absolute_first_column;
    } else {
        if (command->delta_lines < 0) {
            size_t delta = (size_t)(-command->delta_lines);
            if (delta > self->first_visible_line) {
                self->first_visible_line = 0;
            } else {
                self->first_visible_line -= delta;
            }
        } else if (command->delta_lines > 0) {
            self->first_visible_line += (size_t)command->delta_lines;
            if (self->first_visible_line >= self->lines.count) {
                if (self->lines.count > 0) {
                    self->first_visible_line = self->lines.count - 1;
                } else {
                    self->first_visible_line = 0;
                }
            }
        ...
        if (command->delta_columns < 0) {
            size_t delta = (size_t)(-command->delta_columns);
            if (delta > self->first_visible_column) {
                self->first_visible_column = 0;
            } else {
                self->first_visible_column -= delta;
            }
        } else if (command->delta_columns > 0) {
            self->first_visible_column += (size_t)command->delta_columns;
        }
    }
