...
/* keys we care about as commands */
...
/* positions are (line, column) in character cells, 0-based */
...
/* viewport definition (character-space) */
...
/* view selection: clipped to viewport, includes text snapshot */
typedef struct text_editor_view_selection_s {
    text_editor_selection_t range;
    const char *text; /* pointer into temporary view storage */
    size_t byte_length;
...
/* view export: returns arrays valid until next mutating call.
   You pass the viewport; the editor fills out_view. */
